...
double p_dens_station_ave(struct Node nn[ONE_SIDE][ONE_SIDE], int temp_x, int temp_y);
	...
	while(1){
		...
		do{
			...
			while(i<nn[temp->x][temp->y].degree){
				...
				station_ave=p_dens_station_ave(nn,temp->x,temp->y);
				...
				//情報を共有するかどうか判断する
				agree_SIR(nn,nn[temp->x][temp->y].ad_list[i],temp->x,temp->y,&infected_count,station_ave);
				i++;
...
double p_dens_station_ave(struct Node nn[ONE_SIDE][ONE_SIDE], int temp_x, int temp_y){
	...
	for(i=0 ; i<nn[temp_x][temp_y].degree ; i++){
		ave=ave+nn[temp_x][temp_y].station_number;
...
//	printf("edgedot=%d,launch_x=%d,launch_y=%d\ONE_SIDE",edgedot,launch_x,launch_y);
		...
		if(nn[edgedot/ONE_SIDE][edgedot%ONE_SIDE].statement=='S' && nn[edgedot/ONE_SIDE][edgedot%ONE_SIDE].height != 1){
			...
			if(inf_pro + correction<=INFECTED_PROBABIRITY){
				(*infected)++;
