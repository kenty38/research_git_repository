		...
		make_ad_list(node,edge);
			...
			for(i=0;i<PREFECTURE_NUMBER;i++){
				...
				for(temp_pref=0;temp_pref<PREFECTURE_NUMBER-i;temp_pref++){
					...
					if(first_shared > 0 && temp_pref == PREFECTURE_NUMBER-(i+1)) //初期共有者1人以上 かつ もうひとつの県が沖縄に来た時
						break;
          ...
          if(head->x == 0 && head->y == 14)
            TraverseList(head);
					...
					//次の拡散のための初期化
					init_for_next(node,first_shared);
...
void confirm(void){
	...
	puts("0:original, 1:density, 2:station, 3:both");
	printf("->");
	scanf("%d", &mode);	

	remove("../data/result/infected_time=1.txt");
...
void display(struct Node nn[ONE_SIDE][ONE_SIDE]){
	...
	for(i=0;i<ONE_SIDE;i++){
		for(j=0;j<ONE_SIDE;j++)
			printf("node[%d][%d].kind_of_source=%d\n",i,j,nn[i][j].kind_of_source);
	}
